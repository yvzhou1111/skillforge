...
import { lintSkill } from "../../core/linter.js";
...
export async function cmdLint(args: ParsedArgs): Promise<number> {
  const ref = args._[0];
  ...
  if (!ref && !localPath) {
    log.error('Usage: skillforge lint <skill-id|owner/repo>   OR   skillforge lint --dir ./path');
    return 1;
  }
  ...
  const report = lintSkill(materialized);
  ...
  renderLint(report);
  // Exit 2 for failing-grade skills so CI / scripts can gate on it.
  return report.grade === "F" ? 2 : 0;
}
